...
class TestGeospatial extends AnyFunSuite with BeforeAndAfterAll {
  ...
  test("test geospatial") {
    val spark = SparkSession.builder()
      ...
      .config("spark.sql.extensions", "org.apache.sedona.sql.SedonaSqlExtensions,org.apache.iceberg.spark.extensions.IcebergSparkSessionExtensions")
      .getOrCreate()

    spark.sql("CREATE OR REPLACE TABLE local.tmp.geom_table (id INT, geom GEOMETRY) USING iceberg").show()
    spark.sql("""
